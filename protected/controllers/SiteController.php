...
class SiteController extends Controller
{
    ...
    public function actionMailT()
    {
        ...
        $headers .= 'From: tttanyattt@mail.ru'. "\r\n";
        $this->smtpmail('tttanyattt@mail.ru', 'Тема письма', 'Текст письма', $headers);
    }
        ...
        public function server_parse($socket, $response, $line = __LINE__) {
            ...
            while (@substr($server_response, 3, 1) != ' ') {
                if (!($server_response = fgets($socket, 256))) {
                    ...
                    //if ($config['smtp_debug']) echo '<p>Проблемы с отправкой почты!</p>'.$response.'222'.$line.'1111';
                    return false;
                }
            }
            if (!(substr($server_response, 0, 3) == $response)) {
                if ($config['smtp_debug']) echo '<p>Проблемы с отправкой почты!</p>'.$response.'<br>'.$line.'<br>';
                //if ($config['smtp_debug']) echo '<p>Проблемы с отправкой почты!</p>'.$response.'333'.$line.'44';
                return false;
            }
            return true;
        }
