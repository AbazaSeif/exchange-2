	...
	public function safeUp()
	{
		...
		if ($driver=='sqlite') {
			...
			$this->createTable('nfy_messages', array(
				...
				'user_id'=>'integer not null'.($driver=='sqlite' ? ' CONSTRAINT nfy_messages_user_id_fkey REFERENCES '.$userTable.'('.$userPk.') ON DELETE CASCADE ON UPDATE CASCADE' : ''),
				'logtime'=>'timestamp',
				'message'=>'text',
			));
			...
			$this->createTable('nfy_subscriptions2', array(
				'id'=>'pk',
				'channel_id'=>'integer not null CONSTRAINT nfy_subscriptions_channel_id_fkey REFERENCES nfy_channels(id) ON DELETE CASCADE ON UPDATE CASCADE',
				'user_id'=>$userPkType.' not null CONSTRAINT nfy_subscriptions_user_id_fkey REFERENCES '.$userTable.'('.$userPk.') ON DELETE CASCADE ON UPDATE CASCADE',
				'transports'=>'text',
			...
			$this->addForeignKey('nfy_messages_user_id_fkey', 'nfy_messages', 'user_id', $userTable, $userPk, 'CASCADE', 'CASCADE');
		...
		$this->createIndex('nfy_messages_user_id_idx', 'nfy_messages', 'user_id');
	...
	public function safeDown()
	{
		...
		if ($driver=='sqlite') {
			$this->createTable('nfy_queues2', array(
				'id'=>'pk',
				'subscription_id'=>'integer not null CONSTRAINT nfy_queues_subscription_id_fkey REFERENCES nfy_subscriptions(id) ON DELETE CASCADE ON UPDATE CASCADE',
				'message_id'=>'integer not null CONSTRAINT nfy_queues_message_id_fkey REFERENCES nfy_messages(id) ON DELETE CASCADE ON UPDATE CASCADE',
			...
			$this->createTable('nfy_messages2', array(
				'id'=>'pk',
				'channel_id'=>'integer not null CONSTRAINT nfy_messages_channel_id_fkey REFERENCES nfy_channels(id) ON DELETE CASCADE ON UPDATE CASCADE',
				'logtime'=>'timestamp',
				'message'=>'text',
			));
