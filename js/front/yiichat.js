var YiiChat = function(options){
			...
			success: function(_posts){
				if(_posts == null || _posts == ""){
					...
					// _posts.chat_id, _posts.identity, _posts.posts
                                        $('.posts').html('');
                                        if(_posts.posts.length > 0){
                                            //$('#no-offers').html('');
                                            jQuery.each(_posts.posts, function(k,post){
                                                    add(post);
                                            });
                                        } else {
                                            posts.append("<div id='no-offers'>"
                                                + "Сделайте ваше предложение"
                                                + "</div>")
                                            ;
                                            $('.posts').css('overflowY','auto');
                                        }	
	...
	var add = function(post){
		...
		//posts.append("<div id='"+post.field_id+"' class='post'>"
		posts.prepend("<div id='"+post.field_id+"' class='post'>"
			+ "<div class='track'></div>"
			+ "<div class='text'></div>"
		        + "</div>");
                
		//var p = posts.find(".post[id='"+post.id+"']");
		var p = posts.find(".post[id='"+post.field_id+"']");
		p.data('post',post);
		var flag=false;
	...
	chat.html("<div class='you'>you</div><div class='posts'>posts</div><div class='log'></div>");
	...
	//you.html("<input id='price'  readonly = 'readonly' onfocus='this.blur();' style='background-color: #72A4D2;' type='text' value='666' size='2'></input><button>"+options.sendButtonText+"</button><div class='exceded'></div><br/>");	
	you.html("<input id='price' style='background-color: #72A4D2;' type='text' value='666' size='3'></input><button>"+options.sendButtonText+"</button><div class='exceded'></div><br/>");	
